  ...
  vcfFind(ci, nodeBudget = 3e4, maxDepth = 28) {
    ...
    function attack(movesLeft, rootMove) {
      ...
      if (eng.wCount[ci] > 0) {
        ...
        win2.cell = rootMove === -1 ? fiveCell : rootMove;
        return true;
      }
...
function findBestMove(board, player, difficulty, mode, historyLength, persist = true) {
  ...
  if (difficulty === 1) {
    const r2 = engine.rootSearch(ci, 1, cfg.limit);
    const easyNodes = engine.nodes;
    const scored = r2.scored.slice(0, 5).map((m) => toMove(m.cell, Math.round(m.s)));
    const pool = scored.length > 3 && Math.random() < 0.35 ? scored.slice(1) : scored;
    const pick = pool[Math.random() * pool.length | 0] || toMove(r2.best);
    return { move: pick, depth: 1, nodes: easyNodes, ms: Math.round(now() - t0), eval: Math.round(r2.bestV), scores: scored };
  }
  let bestDepth = 0;
  let bestCell = -1;
  let bestV = 0;
  ...
  for (let d = 2; d <= cfg.depth; d++) {
    ...
    const r2 = engine.rootSearch(ci, d, Math.min(cfg.limit, 16));
    if (engine.aborted || r2.best < 0) break;
    bestDepth = d;
    bestCell = r2.best;
    bestV = r2.bestV;
    bestScores = r2.scored.slice(0, 6).map((m) => toMove(m.cell, Math.round(m.s)));
    if (Math.abs(bestV) >= MATE - 64) break;
    if (now() - t0 > timeMs * 0.45) break;
  }
  if (bestCell < 0) {
    const r2 = engine.rootSearch(ci, 1, cfg.limit);
    bestCell = r2.best >= 0 ? r2.best : engine.winCell(ci) >= 0 ? engine.winCell(ci) : -1;
...
function demonMove(b, player) {
  ...
  if (lesson && m && m.x === lesson.x && m.y === lesson.y) {
    ...
    if (alt) {
      fired = `\u6B64\u5C40\u9762(${lesson.x},${lesson.y})\u4E3A\u524D\u6B21\u8D25\u624B\xD7${lesson.count} \u2192 \u6539\u8D70(${alt.x},${alt.y})`;
      m = alt;
    } else {
...
\u2550\u2550\u2550\u2550\u2550\u2550 ${label} \u2550\u2550\u2550\u2550\u2550\u2550`);
  ...
  while (full.length < 120) {
    ...
    if (turn === humanColor) {
      ...
      const r2 = demonMove(b, turn);
      m = r2.m;
      if (r2.fired) fires.push(`  \u{1F4D6} \u7B2C${full.length + 1}\u624B [\u6076\u9B54${turn === 1 ? "\u9ED1" : "\u767D"}(${m.x},${m.y})] ${r2.fired}`);
    }
    b[m.y][m.x] = turn;
  ...
  const w = winner === 0 ? "\u548C\u68CB" : winner === humanColor ? "\u4EBA\u7C7B\u80DC \u{1F389}" : "\u6076\u9B54\u80DC \u{1F608}";
  console.log(`\u7ED3\u679C: ${w} \xB7 \u5171${full.length}\u624B`);
  return { winner, full, humanMoves };
}
var win = null;
for (let attempt = 1; attempt <= 5; attempt++) {
  const g = playGame(1, null, `\u6311\u6218\u5C40 ${attempt} \xB7 \u4EBA\u6267\u9ED1 vs \u6076\u9B54\u767D`);
  if (g.winner === 1) {
    win = g;
    break;
  }
...
var op = detectWinningOpening(win.full, 1);
var r = recordLoss(win.full, 1, 4, op?.name ?? null);
console.log(`
\u{1F4C9} \u6076\u9B54\u590D\u76D8\u5165\u5E93\uFF1A\u8D25\u5C40#${r.losses}${op ? `\uFF08\u300C${op.name}\u300D${op.exact ? "\u5FC5\u80DC\u5B9A\u5F0F" : "\u8D77\u624B\u5F0F"}\uFF09` : ""} \xB7 \u6559\u8BAD\u5E93 ${r.lessons} \u6761`);
var g2 = playGame(1, win.humanMoves, "\u91CD\u653E\u5C40 \xB7 \u4EBA\u7C7B\u539F\u6837\u590D\u523B\u80DC\u5C40\u68CB\u8C31");
if (g2.winner === 1) {
  const op2 = detectWinningOpening(g2.full, 1);
  const r2 = recordLoss(g2.full, 1, 4, op2?.name ?? null);
  console.log(`\u{1F4C9} \u4E8C\u6B21\u5165\u5E93\uFF1A\u8D25\u5C40#${r2.losses} \xB7 \u6559\u8BAD\u5E93 ${r2.lessons} \u6761`);
}
var g3 = playGame(1, win.humanMoves, "\u518D\u91CD\u653E \xB7 \u89C2\u5BDF\u591A\u8F6E\u5B66\u4E60\u540E\u7684\u62B5\u6297");
console.log("\n\u2550\u2550\u2550\u2550\u2550\u2550 \u6863\u6848\u603B\u89C8 \u2550\u2550\u2550\u2550\u2550\u2550");
console.log(`\u603B\u8D25\u5C40 ${getLosses().length} \xB7 \u6559\u8BAD ${lessonCount()} \u6761`);
for (const l of getLosses()) {
  console.log(`  ${new Date(l.ts).toLocaleTimeString()} \xB7 \u4EBA\u7C7B\u6267${l.human === 1 ? "\u9ED1" : "\u767D"} \xB7 ${l.moves.length}\u624B \xB7 ${l.opening ?? "\u672A\u8BC6\u522B\u5F00\u5C40"}`);
}
