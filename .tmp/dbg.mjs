...
var GomokuEngine = class {
  ...
  genMoves(ci, width, first) {
    const out = [];
    const oi = 1 - ci;
    const cs = this.cStack, cn = this.cCount, cells = this.cells;
    ...
    for (let i = 0; i < cn; i++) {
      ...
      const s = atk + def * 0.92 + (p === first ? 1e15 : 0) + (hist[p] + histO[p]) / 64 + this.inf[p] * 2;
      out.push({ cell: p, s });
    }
    out.sort((a, b2) => b2.s - a.s);
  ...
  negamax(ci, depth, alpha, beta, ply, extBudget) {
    ...
    const moves = this.genMoves(ci, width, ttBest);
    if (moves.length === 0) return this.evalFor(ci);
    const origAlpha = alpha;
    let best = -Infinity;
    let bestCell = moves[0].cell;
    let first = true;
    for (const mv of moves) {
      const p = mv.cell;
      const isKiller = p === this.killers[ply * 2] || p === this.killers[ply * 2 + 1];
      ...
      if (this.hasFive(ci)) {
        ...
        let d2 = depth - 1;
        let e2 = extBudget;
        const isFour = this.wCount[ci] > 0;
        if (d2 > 0 && e2 > 0 && (isFour || mv.s >= 11e3)) {
          d2 += 1;
          e2 -= 1;
        }
        if (first) {
  ...
  rootSearch(ci, depth, width) {
    ...
    const moves = this.genMoves(ci, width, first);
    let best = -Infinity;
    let bestCell = moves.length ? moves[0].cell : -1;
    for (const mv of moves) {
      const p = mv.cell;
      const preAtk = mv.s;
      this.makeCell(p, ci + 1);
      ...
      if (this.hasFive(ci)) v = MATE - 1;
        ...
        let d2 = depth - 1;
        let e2 = 6;
        if (d2 > 0 && (this.wCount[ci] > 0 || preAtk >= 11e3)) {
          d2 += 1;
          e2 -= 1;
        }
    ...
    const scored = moves.filter((m) => m.v !== void 0).map((m) => ({ cell: m.cell, s: m.v }));
...
function findBestMove(board, player, difficulty, mode, historyLength, persist = true) {
  ...
  if (difficulty >= 3 && historyLength >= 6) {
    engine.deadlineForVcf(t02 + Math.min(900, timeMs * 0.35));
    const killCell = engine.vcfFind(ci, difficulty === 4 ? 6e4 : 3e4, difficulty === 4 ? 30 : 22);
    if (killCell >= 0) {
      const mv2 = toMove(killCell, MATE - 2);
      return { move: mv2, depth: cfg.depth, nodes: engine.vcfNodes(), ms: Math.round(now() - t02), eval: MATE - 2, scores: [{ ...mv2, v: MATE - 2 }], instant: true };
    }
    engine.startSearch(t02 + timeMs);
  }
  if (difficulty === 1) {
    const r3 = engine.rootSearch(ci, 1, cfg.limit);
    const easyNodes = engine.nodes;
    const scored = r3.scored.slice(0, 5).map((m) => toMove(m.cell, Math.round(m.s)));
    const pool = scored.length > 3 && Math.random() < 0.35 ? scored.slice(1) : scored;
    const pick = pool[Math.random() * pool.length | 0] || toMove(r3.best);
    return { move: pick, depth: 1, nodes: easyNodes, ms: Math.round(now() - t02), eval: Math.round(r3.bestV), scores: scored };
  }
  let bestDepth = 0;
  let bestCell = -1;
  let bestV = 0;
  ...
  for (let d = 2; d <= cfg.depth; d++) {
    if (now() > t02 + timeMs) break;
    const r3 = engine.rootSearch(ci, d, Math.min(cfg.limit, 16));
    if (engine.aborted || r3.best < 0) break;
    bestDepth = d;
    bestCell = r3.best;
    bestV = r3.bestV;
    bestScores = r3.scored.slice(0, 6).map((m) => toMove(m.cell, Math.round(m.s)));
    if (Math.abs(bestV) >= MATE - 64) break;
    if (now() - t02 > timeMs * 0.45) break;
  }
  if (bestCell < 0) {
    const r3 = engine.rootSearch(ci, 1, cfg.limit);
    bestCell = r3.best >= 0 ? r3.best : engine.winCell(ci) >= 0 ? engine.winCell(ci) : -1;
...
var b = empty();
for (const [x, y, c] of [[7, 7, 1], [7, 8, 1], [8, 8, 1], [2, 2, 2], [12, 2, 2], [2, 12, 2]]) b[y][x] = c;
var t0 = Date.now();
var r = findBestMove(b.map((r3) => [...r3]), 2, 4, "ai", 6);
console.log("level4:", JSON.stringify(r), "wall", Date.now() - t0);
var r2 = findBestMove(b.map((r3) => [...r3]), 2, 4, "ai", 6);
