 ...
 *  + BGM playback (demon mode / 斗地主) via media elements.
 * ──────────────────────────────────────────────────────────── */

import demonBgmUrl from '../assets/demon-bgm.m4a';

/** 斗地主牌桌 BGM。放在 public/ 下，由 Service Worker 按需缓存。 */
const DDZ_BGM_URL = '/ddz/bgm/ddz-theme.mp3';

export class AudioEngine {
  private ctx: AudioContext | null = null;
  private _enabled = true;
  private _bgm: HTMLAudioElement | null = null;
  private _ddzBgm: HTMLAudioElement | null = null;
    ...
    try {
      ...
      if (this._bgm.paused) {
        this._bgm.play().catch(() => {
          /* autoplay blocked — will retry on gesture */
        });
      }
    } catch {
      /* noop */
  ...
  /** 斗地主牌桌 BGM。与恶魔主题互不影响，各自独立通道。 */
  startDdzBgm(): void {
    try {
      if (!this._ddzBgm) {
        this._ddzBgm = new Audio(DDZ_BGM_URL);
        this._ddzBgm.loop = true;
        this._ddzBgm.volume = 0.4;
        this._ddzBgm.preload = 'auto';
      }
      if (this._ddzBgm.paused) {
        this._ddzBgm.play().catch(() => {
          /* 自动播放被拦：等下一次点击再试 */
        });
      }
    } catch {
  ...
  stopDdzBgm(): void {
    try {
      if (this._ddzBgm && !this._ddzBgm.paused) this._ddzBgm.pause();
    } catch {
      /* noop */
    }
  }
  ...
  bad(): void {
    this.beep(180, 0.08, 'square', 0.08);
  }
  check(): void {
    this.beep(880, 0.12, 'sawtooth', 0.1);
  }
  win(): void {
